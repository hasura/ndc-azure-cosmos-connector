...
export type NamedScalarTypeDefinition =
  | CustomNamedScalarTypeDefinition
  | BuiltInScalarTypeDefinition;

export type BuiltInScalarTypeDefinition =
  | StringScalarTypeDefinition
  | BooleanScalarTypeDefinition
  | IntegerScalarTypeDefinition
  | NumberScalarTypeDefinition
...
export function getNdcSchemaResponse(
  ...
  const objectTypes = mapObjectValues(
    collectionsSchema.objectTypes,
    (objDef) => {
      return {
        fields: Object.fromEntries(
          Object.values(objDef.properties).map((propDef) => {
...
function convertTypeReferenceToSdkType(typeRef: TypeDefinition): sdk.Type {
  switch (typeRef.type) {
    case "array":
      return {
        type: "array",
        element_type: convertTypeReferenceToSdkType(typeRef.elementType),
      };
    case "nullable":
      return {
        type: "nullable",
        underlying_type: convertTypeReferenceToSdkType(typeRef.underlyingType),
      };
    case "named":
      return { type: "named", name: typeRef.name };
  }
}
...
export function getBaseNamedType(typeRef: TypeDefinition): string {
  switch (typeRef.type) {
    case "array":
      return getBaseNamedType(typeRef.elementType);
    case "nullable":
      return getBaseNamedType(typeRef.underlyingType);
    case "named":
      return typeRef.name;
  }
}
