...
**/
export async function fetchLatestNRowsFromContainer(
  n: number,
  container: Container,
): Promise<string[]> {
  const querySpec = {
...
export async function inferJSONSchemaFromContainerRows(
  rows: string[],
...
function getPropertyTypeDefn(
  property: JSONSchemaProperty,
  $refs: $RefParser,
): TypeDefinition | null {
  if (property.$ref !== undefined && property.$ref !== null) {
    const referencedPropertyDefn = $refs.$refs.get(
      property.$ref,
    ...
    if (referencedPropertyDefn.type === "object") {
      ...
      console.log(
        "Warning: Could not infer the type for referenced property",
        property,
      );
    }
...
export async function getObjectTypeDefinitionsFromJSONSchema(
  containerJSONSchema: JSONSchema,
): Promise<ObjectTypeDefinitions> {
  var objectTypeDefinitions: ObjectTypeDefinitions = {};
  let parser = new $RefParser();

  const $refs = await parser.resolve(
    JSON.parse(JSON.stringify(containerJSONSchema)),
  );
  Object.entries(containerJSONSchema.definitions).forEach(
    ([objectTypeName, objectTypeDefinition]) => {
      if (objectTypeDefinition.type == "object") {
        ...
        if (objectTypeDefinition.properties !== undefined) {
          Object.entries(objectTypeDefinition.properties).map(
            ([propertyName, propertyDefn]) => {
              ...
              if (
                propertyTypeDefn !== null &&
                !legacyProperty.includes(propertyName)
              ) {
...
async function getCollectionsSchema(
  ...
  const { resources: allContainers } = await database.containers
    .readAll()
    .fetchAll();
  ...
  for (const container of allContainers) {
    ...
    const nContainerRows = await fetchLatestNRowsFromContainer(
      nRows,
      dbContainer,
    );
    ...
    const containerObjectTypeDefinitions =
      await getObjectTypeDefinitionsFromJSONSchema(containerJsonSchema);
    ...
    objectTypeDefinitions = {
      ...objectTypeDefinitions,
      ...containerObjectTypeDefinitions,
    };
